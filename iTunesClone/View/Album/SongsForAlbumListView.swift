    ...
    var body: some View {
        Grid(horizontalSpacing: 20){
            ForEach(songs){ song in
                ...
                .foregroundStyle(selectedSong?.id == song.id ?Color.accentColor : Color.black)
                .id(song.trackNumber)
        ...
        VStack(spacing: 10){
            ForEach(songs){  song in
                HStack(spacing: 10){
                    ...
                    BuyButton(urlString: song.artworkUrl100, price: song.trackPrice, currency: song.currency)
                        .padding(.trailing)
                }
                .foregroundStyle(selectedSong?.id == song.id ? Color.accentColor : Color.gray)
                .id(song.trackNumber)
...
fileprivate func formattedDuration(time: Int) ->String {
    let timeSeconds = time / 100
    
    let interval = TimeInterval(timeSeconds)
